...
// 	precachemodel( "com_trashbag" );
	...
	//moved to load.gsc
	//level.player thread maps/pointsystem::xp_init();
...
// for me, execute scripts on each spawned enemy
spawn_think_action( targetname )
{
...
//		level thread friendly_waittill_death( self );
	...
	//Atttach immersive scripts to AI. -spe
	//very hardcoded team?
	if(self.team == "axis"){
		self thread maps\_damagefeedback::monitorDamage();
		self thread maps\_damagefeedback::monitorFlashBang();
	}
	//this is done in its own script
	//self thread maps\pointsystem::monitorHurt();
	
	//this too
	// player score stuff
	//self thread maps\pointsystem::player_score_think();
