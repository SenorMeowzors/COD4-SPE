...
friendly_fire_think( entity )
{
	...
	for (;;)
	{
		...
		friendly_fire_checkPoints(entity);
	}
}
...
friendly_fire_checkPoints(entity)
{
	if ( level.player.participation <= ( level.friendlyfire[ "min_participation" ] ) )
	{
		...
		if (isDefined(entity) && isDefined( entity.magic_bullet_shield) &&  entity.magic_bullet_shield )
		{
		entity stop_magic_bullet_shield();
		entity DoDamage(entity.health + 100000,(0,0,0));
