...
init()
{
    ...
    level.player.isInLastStand = false;
	level.raygunCheat = false;
...
specialFeaturesMenu()
{
	...
	addCheat( "sf_use_raygun", ::raygunMode );
	addCheat( "sf_use_stockoption", ::stockOption );
...
stockOption( cheatvalue )
{

	if(cheatValue)
	{
		level.player thread semiInfiniteAmmo();
	}
	else
	{
		level notify ( "stockoptionEnd" );
	}
}

semiInfiniteAmmo( cheatValue )
{

	level endon( "stockoptionEnd" );
	...
	for ( ;; )
	{
		...
		currentAmmoStock = level.player GetWeaponAmmoStock( currentweapon );
		currentAmmoClip = level.player GetCurrentWeaponClipAmmo();

		if(currentammoStock > 1)
		{
			level.player SetWeaponAmmoClip(currentweapon,currentAmmoClip + 1);
		}
		level.player SetWeaponAmmoStock(currentweapon,currentAmmoStock-1);
...
raygunMode( cheatValue )
{
	if ( cheatValue )
	{
		//SetDvar("player_deathInvulnerableToMelee", 1);
		level.player thread healthMonitor();
		//initialize this var
		level.player.lastStandWeaponsList = level.player GetWeaponsList();
		rayGunAttachToSpawners();
		rayGunAttachToExistingSpawns();
	}
	else
	{
		//SetDvar("player_deathInvulnerableToMelee", 0);
		level notify ( "raygun_end" );	
	}
}
...
rayGunAttachToExistingSpawns()
{
	...
	for ( i = 0; i < aiarray.size; i++ )
	{
		...
		if ( isdefined( ai ) )
		{
			//iprintlnbold("attached!");
			ai thread monitorGuy();
		}	
	}
...
rayGunAttachToSpawners()
{
	//Get ALL spawners and attach a monitor to each enemy.
	spawners = getspawnerarray();
	for ( i = 0;i < spawners.size;i++ )
	{
		spawners[ i ] thread attachToGuys();
	}
}
...
healthMonitor()
{
    ...
    for(;;)
    {
		...
		//Do the last stand down thing
        if(level.player.isInLastStand == false && level.player.health <= 5)
        {
            if ( isalive( level.player ) == false ) 
            return;

            level.player.isInLastStand = true;
            ...
            self AllowSprint(0);
            level.player giveweapon( "raygun" );
            level.player giveMaxAmmo( "raygun" );
            level.player switchtoWeapon( "raygun" );
	        self thread lastStandTimer( 5 );
        }
    }
...
lastStandTimer( delay )
{
    ...
    if(level.player.isInLastStand == true){
...
selfMedication()
{
    ...
    level.player TakeWeapon( "raygun" );

    //return weapons
    for( i = 0; i < level.player.lastStandWeaponsList.size; i++ )
    {
        self GiveWeapon(level.player.lastStandWeaponsList[i]);
    }


    level.player.isInLastStand = false;
    self SwitchToWeapon(level.player GetWeaponsListPrimaries()[0]);
...
wait_for_grenades()
{
	...
	while(1)
	{
		...
		level.player thread create_clusterGrenade();
	}
}
...
getClusterGrenadeVelocity()
{
	...
	speed = randomFloatRange( 0, 3000); // play with these values
